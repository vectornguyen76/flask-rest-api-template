...
@click.option("--pattern", default='tests_*.py', help='Test search pattern', required=False)
def cov(pattern):
    """
...
def init_db_user():
    ...
    db.session.add_all([role_permission_admin1, role_permission_admin2, role_permission_admin3,
                        role_permission_user1, role_permission_user2, role_permission_guest])
    db.session.commit()
...
def init_app(app):
    if app.config['APP_ENV'] == 'production':
        ...
        commands = [create_db, reset_db, drop_db, create_user_admin, tests, cov_html, cov]
